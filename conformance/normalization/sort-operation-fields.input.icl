// Conformance: normalization/sort-operation-fields.input.icl
// Tests: Fields within an operation should be sorted alphabetically
// Normalize → normalization/sort-operation-fields.expected.icl

Contract {
  Identity {
    stable_id: "ic-norm-15-001",
    version: 1,
    created_timestamp: 2026-02-01T00:00:00Z,
    owner: "normalization-test",
    semantic_hash: "0000000000000000"
  }

  PurposeStatement {
    narrative: "Test sorting of fields within an operation",
    intent_source: "conformance_test",
    confidence_level: 1.0
  }

  DataSemantics {
    state: {
      counter: Integer = 0
    },
    invariants: ["counter >= 0"]
  }

  BehavioralSemantics {
    operations: [
      {
        side_effects: ["audit_log"],
        postcondition: "counter incremented by amount",
        name: "increment",
        idempotence: "not_idempotent",
        precondition: "counter >= 0",
        parameters: {
          amount: Integer
        }
      }
    ]
  }

  ExecutionConstraints {
    trigger_types: ["manual"],
    resource_limits: {
      max_memory_bytes: 1048576,
      computation_timeout_ms: 100,
      max_state_size_bytes: 1048576
    },
    external_permissions: [],
    sandbox_mode: "full_isolation"
  }

  HumanMachineContract {
    system_commitments: [],
    system_refusals: [],
    user_obligations: []
  }
}
